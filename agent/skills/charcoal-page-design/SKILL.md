...
11. Run the full app only for navigation, route history, platform integration, native input, and end-to-end journeys. Executable runtime scenarios must collectively visit every inventoried surface, including secondary destinations such as Profile. For controlled navigation, verify the first frame after selection—not only the settled frame—so the previous item, next item, selected semantics, content, and route state cannot disagree.
